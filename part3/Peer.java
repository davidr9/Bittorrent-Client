...
public class Peer implements Runnable{
	...
	public int shakeHand() throws IOException, SocketException{
		...
		System.out.println("HANDSHAKE WITH PEER AT " + IPAddress + " COMPLETE");
	...
	private void downloadPieces() throws EOFException, IOException, NoSuchAlgorithmException{
	 ...
	 public void acceptMessage() throws IOException {
	        ...
	        if (peerResponse.message_id == Message.bitfield) {
	            ...
	            HaveMessage hMessage = (HaveMessage) peerResponse;
	            System.out.println("HAVE MESSAGE FOR " + hMessage.getPieceIndex() +" received from peer at " + IPAddress);
	            whichPieces[hMessage.getPieceIndex()] = true;
	        } else {
		...
		for (int i = 0; i < newPiece.totalBlocks; i++){
			...
			if (peerResponse.message_id == Message.piece){
				...
				if (peerPiece.getPieceIndex() != piece){
					...
					System.err.println("Peer responded with wrong block from requested piece");
					i--;
					continue;
				} else {
